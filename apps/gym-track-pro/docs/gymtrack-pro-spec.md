  ...
  const colorMap = useMemo(() => {
    ...
    sessions?.forEach(session => {
      session.session_muscle_groups.forEach(smg => {
        if (smg.is_completed) {
          const daysSinceTraining = differenceInDays(new Date(), new Date(smg.completed_at));
          // Músculo reciente = verde brillante, más antiguo = verde apagado
          map[smg.muscle_groups.slug] = daysSinceTraining === 0
...
```css
--color-bg:          #0f0f0f    /* Negro profundo */
--color-surface:     #1a1a1a    /* Superficie de tarjetas */
--color-border:      #2a2a2a    /* Bordes sutiles */
--color-accent:      #a3e635    /* Verde lima — acción principal */
--color-accent-dim:  #65a30d    /* Acento oscuro */
--color-text:        #f5f5f5    /* Texto principal */
--color-text-muted:  #737373    /* Texto secundario */
...
*Documento generado para GymTrack Pro — Versión 1.0*
