...
export function calculateBMI(stats: BodyStats, activityFactor = 1.55): BMIResult {
  ...
  if      (bmi < 18.5) { category = 'underweight'; categoryLabel = 'Bajo peso' }
  else if (bmi < 25)   { category = 'normal';      categoryLabel = 'Peso normal' }
  else if (bmi < 30)   { category = 'overweight';  categoryLabel = 'Sobrepeso' }
  else                 { category = 'obese';        categoryLabel = 'Obesidad' }
...
import { SILHOUETTES } from './silhouettes'  // objeto con los 10 SVGs
...
export function BMIPage() {
  ...
  return (
    <div>
      ...
      <BMISilhouette silhouetteKey={stats.silhouetteKey} gender={profile.gender} />
      <StatsGrid result={stats} profile={profile} />
      <WeightHistory measurements={measurements} />
      <UpdateWeightForm weight={weight} onChange={setWeight} onSave={saveMeasurement} loading={saving} />
    </div>
  )
}
```
...
async function importExercises() {
  ...
  for (let i = 0; i < exercises.length; i += BATCH_SIZE) {
    ...
    const rows = batch.map((ex: any) => {
      ...
      const targetSlug = TARGET_MAP[ex.target?.toLowerCase()] ??
...
*GymTrack Pro — Especificación Técnica v2.0 — Abril 2025*
*No hay soft delete. Los DELETE son permanentes. La auditoría en `audit_log` permite reconstruir datos si es necesario.*
