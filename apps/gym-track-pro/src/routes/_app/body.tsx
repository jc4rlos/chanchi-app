...
import { useProfile, useBodyMeasurements, useSaveMeasurement } from '@/features/profile/use-body'
...
const BodyPage = () => {
  ...
  const tmb = latestWeight && heightCm ? calcTMB(latestWeight, heightCm, age, isFemale) : null
    ...
    <div className='flex flex-col gap-3.5 pb-10'>
      ...
      {bmi && (
        <div
          ...
          style={{ background: 'linear-gradient(135deg,#111d00,#0a1200)', borderColor: '#2a4a1a' }}
        >
          <div className='flex items-start justify-between'>
            <div>
              <p className='text-[11px] font-semibold uppercase tracking-wide text-[#4a7a4a]'>
            ...
            <div className='text-right'>
              <p className='text-[26px] font-bold text-foreground'>{latestWeight} kg</p>
              {heightCm && <p className='text-[13px] text-muted'>{heightCm} cm</p>}
              <p className='mt-1 text-[11px] text-[#4a7a4a]'>
                {measurements.length > 0 ? `Actualizado ${lastMeasuredDate}` : 'Sin medición'}
              </p>
            </div>
          </div>
        </div>
      ...
      {scalePercent !== null && (
        <div className='rounded-[16px] border border-[#1e1e1e] bg-[#131313] p-3.5'>
          ...
          <div className='relative h-2 w-full overflow-visible rounded-full'>
            <div className='h-full w-full rounded-full' style={{ background: BMI_SCALE_COLOR }} />
            <div
      ...
      {bmi && (
        <div className='flex gap-3'>
          <div className='flex min-w-[120px] flex-col items-center gap-2 rounded-[16px] border border-[#1e1e1e] bg-[#131313] p-3'>
            ...
            <p className='text-center text-[10px] font-semibold leading-tight' style={{ color: '#4a7a4a' }}>
              IMC {bmi.toFixed(1)}<br />{BMI_LABELS[category]}
            </p>
          </div>
          ...
          <div className='flex flex-1 flex-col gap-2'>
            ...
            {goalCal && (
              ...
              >
                ...
                <p className='mt-0.5 text-[13px] font-bold text-primary'>{goalCal.label}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {!bmi && (
        <div className='rounded-[16px] border border-border bg-card p-6 text-center'>
          <p className='text-[16px] font-bold text-foreground'>Sin datos de cuerpo</p>
      ...
      <div className='rounded-[16px] border border-border bg-card p-3.5'>
        ...
        <div className='flex gap-2.5'>
          <input
            ...
            className='flex-1 rounded-xl border border-border bg-[#1a1a1a] px-4 py-3 text-center text-[26px] font-bold text-foreground placeholder:text-soft focus:border-primary/60 focus:outline-none'
            step={0.1}
            min={20}
            max={300}
          />
