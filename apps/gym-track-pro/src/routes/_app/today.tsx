...
const NoSession = ({ onCreate }: { onCreate: () => void }) => (
  <div className='flex flex-col items-center justify-center gap-4 py-24 text-center'>
    <div className='flex h-16 w-16 items-center justify-center rounded-2xl bg-primary-light'>
      <svg width='28' height='28' viewBox='0 0 24 24' fill='none' stroke='#a3e635' strokeWidth='2' strokeLinecap='round'>
        <line x1='12' y1='5' x2='12' y2='19'/><line x1='5' y1='12' x2='19' y2='12'/>
      </svg>
    </div>
    <div>
      <p className='text-[18px] font-bold text-foreground'>Sin sesión hoy</p>
      <p className='mt-1 text-[13px] text-muted'>Registrá tus grupos musculares y comenzá</p>
    </div>
    <button
      onClick={onCreate}
...
const TodayPage = () => {
      ...
      return (
        <div className='flex flex-col gap-4 pb-10'>
          <div>
            ...
            <p className='text-[13px] text-muted mt-0.5'>{formatDate(new Date())}</p>
          </div>
          <TodayPlanView
            routines={planRoutines!}
            checked={new Set()}
            onToggle={() => {}}
            onExerciseDetail={(id) => navigate({ to: '/exercises/$exerciseId', params: { exerciseId: id } })}
          />
          <button
  ...
  const doneExercises = allPlanExercises.filter((e) => checkedExercises.has(e.id)).length
    ...
    <div className='flex flex-col gap-3.5 pb-10'>
      ...
      <div className='flex items-center gap-3 border-b border-border pb-3'>
        ...
        <div className='flex-1 min-w-0'>
          <p className='text-[15px] font-bold text-foreground capitalize'>{todayLabel}</p>
          <p className='text-[11px] text-muted truncate'>
            {new Date(session.started_at).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })} · {muscleNames || 'Sin grupos aún'}
          </p>
        </div>
        <div className='flex items-center gap-1.5 rounded-full border border-[#2a4a1a] bg-primary-light px-2.5 py-1'>
          {!isFinished && <div className='h-1.5 w-1.5 rounded-full bg-primary' />}
      ...
      <div className='rounded-[18px] border border-[#1e1e1e] bg-[#131313] p-4'>
        <div className='mb-3 flex items-start justify-between'>
          <div>
            <p className='text-[11px] font-medium uppercase tracking-wide text-muted'>Completados</p>
            <p className='mt-0.5 leading-none'>
              <span className='font-display text-[48px] text-primary'>{doneCount}</span>
              <span className='font-display text-[22px] text-[#3a3a3a]'> / {total}</span>
            </p>
          </div>
          <div className='text-right'>
            <p className='font-display text-[28px] leading-none text-foreground'>{fmtTime(elapsed)}</p>
        ...
        <div className='relative h-2 overflow-visible rounded-full bg-[#1a1a1a]'>
          ...
          {pct > 0 && pct < 100 && (
            <div
              className='absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 -translate-x-1/2 rounded-full border-2 border-[#131313] bg-primary'
              style={{ left: `${pct}%` }}
            />
          )}
        </div>
      ...
      {hasPlan ? (
        <div>
          <p className='mb-2.5 text-[11px] font-semibold uppercase tracking-wide text-muted'>Rutinas de hoy</p>
          <TodayPlanView
            routines={planRoutines!}
        ...
        <div>
          <div className='mb-2.5 flex items-center justify-between'>
            <p className='text-[11px] font-semibold uppercase tracking-wide text-muted'>Grupos musculares</p>
            {!isFinished && (
              <button
      ...
      <div className='rounded-[14px] border border-border bg-card p-3.5'>
        <p className='mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted'>Notas</p>
        <textarea
          ref={notesRef}
          defaultValue={session.notes ?? ''}
          onChange={(e) => handleNotesChange(e.target.value)}
          placeholder='Ej: Aumenté press banca a 80 kg...'
          className='w-full resize-none bg-transparent text-[13px] text-foreground placeholder:text-soft focus:outline-none'
          rows={2}
          readOnly={isFinished}
        />
      </div>
        ...
        >
          ...
          <div className='text-left'>
            <p className='text-[15px] font-bold text-primary-foreground'>
              {finishSession.isPending ? 'Guardando...' : 'Finalizar entrenamiento'}
            </p>
            <p className='text-[10px] text-[#1a3a00]'>Marcará el sello del día ✓</p>
          </div>
        </button>
      ) : (
        <div className='flex items-center justify-center gap-2 rounded-[14px] border border-[#2a4a1a] bg-primary-light py-4'>
          <svg width='18' height='18' viewBox='0 0 10 10'>
            <polyline points='1.5,5 4,7.5 8.5,2.5' fill='none' stroke='#a3e635' strokeWidth='1.9' strokeLinecap='round' strokeLinejoin='round'/>
          </svg>
          <p className='text-[15px] font-bold text-primary'>Entrenamiento finalizado</p>
        </div>
      )}
