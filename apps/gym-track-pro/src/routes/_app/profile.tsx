...
import { calcBmi, getBmiCategory, BMI_LABELS, BMI_COLORS } from '@/features/profile/bmi-utils'
import { useProfile, useBodyMeasurements, useSessionStats } from '@/features/profile/use-body'
...
const ProfilePage = () => {
  ...
  return (
    <div className='flex flex-col gap-0 pb-10'>
      {/* Header */}
      <div className='flex items-center gap-3.5 px-0 pb-4 pt-1'>
        ...
        <button className='flex h-[34px] w-[34px] flex-shrink-0 items-center justify-center rounded-full border border-[#2a2a2a] bg-[#1a1a1a]'>
          <svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='#f5f5f5' strokeWidth='2' strokeLinecap='round'>
      ...
      {/* Datos físicos */}
      <div className='mb-3 overflow-hidden rounded-[16px] border border-[#1e1e1e] bg-card-dark'>
        <p className='border-b border-[#1e1e1e] px-4 py-3 text-[11px] font-bold uppercase tracking-[.07em] text-muted'>
          Datos físicos
        </p>
        {/* Género */}
        <div className='flex items-center justify-between border-b border-card-dark px-4 py-3.5'>
        ...
        <div className='flex items-center justify-between border-b border-card-dark px-4 py-3.5'>
          ...
          <span className='text-[13px] text-muted'>{heightCm ? `${heightCm} cm` : '—'}</span>
        </div>
        {/* Peso actual → navigates to body */}
        <button
          className='flex w-full items-center justify-between border-b border-card-dark px-4 py-3.5'
        ...
        >
          ...
          <div className='flex items-center gap-1.5'>
            {bmi ? (
              <>
                <span className='text-[13px] font-bold text-primary'>{bmi.toFixed(1)}</span>
                {bmiLabel && (
                  <span
                    className='rounded-full px-2 py-0.5 text-[11px]'
                    style={{ color: bmiColor, background: '#111d00', border: `1px solid ${bmiColor}40` }}
                  >
                    {bmiLabel.replace(' ✓', '')}
                  </span>
                )}
      ...
      <button
        ...
        style={{ background: 'linear-gradient(135deg,#111d00,#0a1200)', border: '1px solid #2a4a1a' }}
        onClick={() => navigate({ to: '/body' })}
      >
        <div className='flex items-center gap-3'>
          <span className='text-[28px]'>📊</span>
          <div className='text-left'>
            <p className='text-[13px] font-bold text-primary'>IMC y composición corporal</p>
            <p className='text-[11px] text-[#4a7a4a]'>
              {bmi ? `IMC ${bmi.toFixed(1)} · ${bmiLabel?.replace(' ✓', '')}` : 'Ver análisis completo'}
            </p>
          </div>
        </div>
      ...
      <div className='mb-3 overflow-hidden rounded-[16px] border border-[#1e1e1e] bg-card-dark'>
        ...
        <div className='flex divide-x divide-card-dark'>
          <div className='flex flex-1 flex-col items-center py-3.5'>
            <p className='font-display text-[30px] text-primary'>{stats?.totalSessions ?? 0}</p>
            <p className='text-[11px] text-muted'>sesiones</p>
          </div>
          <div className='flex flex-1 flex-col items-center py-3.5'>
            <p className='font-display text-[30px] text-foreground'>{stats?.weeks ?? 0}</p>
      ...
      {/* Version */}
      <div className='mt-2 rounded-[14px] border border-[#1e1e1e] bg-card-dark py-3 text-center'>
        <p className='text-[11px] text-[#3a3a3a]'>GymTrack Pro v2.0.0 · PWA · React + Supabase</p>
      </div>
    </div>
  )
}
