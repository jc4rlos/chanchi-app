...
type Category = 'all' | 'chest' | 'back' | 'arms' | 'legs' | 'shoulders' | 'core'
...
export const useExercisesFiltered = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from('exercises')
        .select('id, name, name_es, body_part, target_muscle, secondary_muscles, equipment, gif_url, instructions, difficulty', { count: 'exact' })
      ...
      if (selectedCategory !== 'all') {
        ...
        if (targets.length > 0) {
          query = query.or(targets.map((t) => `target_muscle.ilike.%${t}%`).join(','))
        }
      }
  ...
  const exercises = (data?.items || []).map((ex) => ({
    ...
    difficulty: (ex.difficulty as 'beginner' | 'intermediate' | 'advanced') || 'beginner',
