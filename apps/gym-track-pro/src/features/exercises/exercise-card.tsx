...
export const ExerciseCard = ({
  ...
  return (
    <button
      onClick={onClick}
      className='flex w-full items-start gap-3 rounded-[14px] border border-border bg-card p-3 text-left transition-colors hover:bg-card-dark'
    >
      <div className='flex h-14 w-14 shrink-0 items-center justify-center rounded-[12px] bg-muscle-rest text-2xl'>
        {emoji}
      </div>
        ...
        <div className='mt-1.5 flex flex-wrap gap-1'>
          {allMuscles.slice(0, 3).map((muscle) => {
            ...
            return (
              <span
                key={muscle}
                className={cn('rounded-[10px] px-2 py-0.5 text-[10px] font-medium', colors.bg, colors.text)}
              >
      ...
      <ChevronRight size={18} className='mt-1 shrink-0 text-soft' />
    </button>
  )
}
