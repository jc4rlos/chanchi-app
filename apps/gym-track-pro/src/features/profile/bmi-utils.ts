export type BmiCategory = 'underweight' | 'normal' | 'overweight' | 'obese' | 'extremely_obese'
...
export const BMI_SCALE_COLOR = 'linear-gradient(90deg, #22d3ee 0%, #a3e635 22%, #facc15 50%, #f97316 72%, #ef4444 100%)'

/** Devine formula ideal weight range [min, max] in kg */
export function calcIdealWeight(heightCm: number, isFemale: boolean): [number, number] {
...
export function calcTMB(weightKg: number, heightCm: number, ageYears: number, isFemale: boolean): number {
...
export function calcTDEE(tmb: number, activityLevel: string | null): number {
  return Math.round(tmb * (ACTIVITY_MULTIPLIERS[activityLevel ?? 'moderate'] ?? 1.55))
}
...
export function getGoalCalories(tdee: number, goal: string | null): GoalCalories {
  if (goal === 'lose_weight') return { label: '−500 cal déficit', value: tdee - 500, delta: -500 }
  if (goal === 'gain_muscle') return { label: '+500 cal superávit', value: tdee + 500, delta: 500 }
