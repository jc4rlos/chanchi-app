...
export const WeightChart = ({ measurements }: Props) => {
  ...
  const diffLabel = diff > 0 ? `+${diff} kg este período` : diff < 0 ? `${diff} kg este período` : 'Sin cambio'
  ...
  return (
    <div className='rounded-[16px] border border-[#1e1e1e] bg-[#131313] p-3.5'>
      <div className='mb-3 flex items-center justify-between'>
        <span className='text-[11px] font-medium text-muted'>Últimas mediciones</span>
      ...
      <div className='flex items-end gap-2' style={{ height: 64 }}>
        {data.map((m, i) => {
          ...
          const barH = Math.max(8, Math.round(((m.weight_kg - minW) / range) * 52))
          return (
            <div key={i} className='flex flex-1 flex-col items-center gap-1'>
              ...
              <span
                className='text-[9px]'
                style={{ color: isLast ? '#a3e635' : '#4a4a4a', fontWeight: isLast ? 600 : 400 }}
              >
        ...
        {data.map((m, i) => {
          ...
          return (
            <span
              ...
              style={{ color: isLast ? '#a3e635' : '#737373', fontWeight: isLast ? 600 : 400 }}
            >
