...
export const MuscleProgress = ({ muscleGroups, weekMuscleSlugs }: Props) => {
  ...
  const trained = muscleGroups.filter((mg) => weekMuscleSlugs.has(mg.slug)).length
  const pct = total > 0 ? Math.round((trained / total) * 100) : 0

  return (
    <div className='rounded-[16px] border border-border bg-card p-3.5'>
      <div className='mb-2.5 flex items-center justify-between'>
        <span className='text-[13px] font-medium text-muted'>Músculos esta semana</span>
          ...
          return (
            <span
              ...
              className={cn(
                ...
                done ? 'bg-primary/15 text-primary' : 'bg-[#1a1a1a] text-[#4a4a4a]'
              )}
            >
              {mg.name_es}
            </span>
