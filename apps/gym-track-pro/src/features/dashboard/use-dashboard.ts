...
export function useWeekSessions() {
  ...
  return useQuery({
    queryKey: ['week_sessions', user?.id, weekStart.toISOString(), weekEnd.toISOString()],
    queryFn: async () => {
      const { data, error } = await supabase
...
export function useDashboardData() {
  ...
  for (const session of sessions) {
    ...
    for (const smg of (session.session_muscle_groups as Array<{ muscle_groups: { slug: string } | null }>) ?? []) {
