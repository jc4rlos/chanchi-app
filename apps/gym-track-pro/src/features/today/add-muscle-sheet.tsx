...
import { useAllMuscleGroups, useAddMuscleToSession, type SessionMuscle } from './use-today'
    ...
    <>
      ...
      <div className='fixed right-0 bottom-0 left-0 z-50 rounded-t-3xl border-t border-border bg-card pb-10'>
        <div className='flex items-center justify-between px-5 py-4'>
          <h2 className='text-[16px] font-bold text-foreground'>Agregar grupo muscular</h2>
          <button onClick={onClose} className='flex h-8 w-8 items-center justify-center rounded-full bg-background text-muted'>
            <X size={16} />
          </button>
        </div>

        {available.length === 0 ? (
          <p className='px-5 pb-4 text-sm text-muted'>Todos los grupos ya están en la sesión.</p>
        ) : (
          <div className='flex max-h-72 flex-col gap-1.5 overflow-y-auto px-5'>
            {available.map((mg) => (
              ...
              >
                <span className='text-[14px] font-medium text-foreground'>{mg.name_es}</span>
