...
export const MuscleItem = ({ item }: Props) => {
  ...
  return (
    <div className={cn('flex items-center gap-3 rounded-[14px] p-3', done ? 'bg-[#0a150a]' : 'bg-card border border-border')}>
      {/* Left accent bar */}
      <div
      ...
      <div className='min-w-0 flex-1'>
        ...
        <p className={cn('text-[11px]', done ? 'text-[#3a5a3a]' : 'text-muted')}>
      ...
      {done ? (
        <div className='flex items-center gap-1.5'>
          ...
          >
            <svg width='10' height='10' viewBox='0 0 10 10'><line x1='2' y1='5' x2='8' y2='5' stroke='currentColor' strokeWidth='1.8' strokeLinecap='round'/></svg>
          </button>
          <div className='flex min-w-9 flex-col items-center rounded-lg border border-[#1a3a1a] bg-[#0a150a] px-2.5 py-1'>
            <span className='font-display text-[17px] leading-none text-[#4ade80]'>{sets}</span>
          </div>
          <button
            onClick={() => handleSets(1)}
            className='flex h-6 w-6 items-center justify-center rounded-full text-[#4ade80] hover:bg-[#1a3a1a]'
          >
            <svg width='10' height='10' viewBox='0 0 10 10'>
              <line x1='5' y1='2' x2='5' y2='8' stroke='currentColor' strokeWidth='1.8' strokeLinecap='round'/>
        ...
        <div className='flex min-w-9 items-center justify-center rounded-lg border border-border bg-background px-2.5 py-1'>
          <span className='font-display text-[17px] leading-none text-border'>—</span>
        </div>
      )}
    </div>
  )
