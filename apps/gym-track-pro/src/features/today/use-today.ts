...
export function useAddMuscleToSession() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async ({ sessionId, muscleGroupId }: { sessionId: string; muscleGroupId: string }) => {
...
export function useUpdateSets() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, setsCount }: { id: string; setsCount: number }) => {
      const { error } = await supabase
        .from('session_muscle_groups')
...
export function useFinishSession() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['today_session', user?.id, getTodayStr()] })
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (targetMuscles.length > 0) {
        const slugs = [...new Set(
          targetMuscles.map((m) => EXERCISE_MUSCLE_TO_SLUG[m.toLowerCase()] ?? m.toLowerCase())
        )]
        ...
        if (groups && groups.length > 0) {
          ...
          await supabase.from('session_muscle_groups').delete().eq('session_id', sessionId)
          const { error: insertErr } = await supabase.from('session_muscle_groups').insert(muscleRows)
