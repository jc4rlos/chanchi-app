...
import type { PlanRoutineWithExercises, PlanExercise } from '@/features/plan/use-weekly-plan'
...
function ExerciseRow({ ex, checked, onToggle, onDetail }: ExerciseRowProps) {
  ...
  return (
    <div className={cn(
      ...
      checked ? 'bg-primary/5 border border-primary/20' : 'bg-background border border-border'
    )}>
      <button
        onClick={onToggle}
        className={cn(
      ...
      >
        {checked && (
          <svg width='11' height='11' viewBox='0 0 10 10'>
            <polyline points='1.5,5 4,7.5 8.5,2.5' fill='none' stroke='#0f0f0f' strokeWidth='1.9' strokeLinecap='round' strokeLinejoin='round' />
          </svg>
        )}
      </button>

      <div className='flex-1 min-w-0'>
        <p className={cn('text-sm font-semibold', checked && 'line-through text-muted')}>{name}</p>
      ...
      {onDetail && ex.exercises?.id && (
        <button
          ...
          className='p-1.5 text-muted hover:text-primary hover:bg-primary/10 rounded-lg transition-colors shrink-0'
        >
          <Info size={15} />
        </button>
      )}
...
export function TodayPlanView({ routines, checked, onToggle, onExerciseDetail }: TodayPlanViewProps) {
          ...
          <div key={r.id} className='bg-card border border-border rounded-2xl overflow-hidden'>
            ...
            >
              ...
              <div className='flex-1 min-w-0'>
                ...
                <p className='text-xs text-muted mt-0.5'>{doneCount}/{exercises.length} ejercicios</p>
              </div>
              <div className={cn(
                ...
                {doneCount === exercises.length && exercises.length > 0 ? '✓ Listo' : `${doneCount}/${exercises.length}`}
              </div>
            </button>
            ...
            {isOpen && exercises.length === 0 && (
              <p className='px-4 pb-4 text-xs text-muted'>Sin ejercicios en esta rutina</p>
            )}
          </div>
        )
      })}
