  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('weekly_plans')
        .select('*, weekly_plan_days(*, weekly_plan_day_routines(*, routines(id, name)))')
        .eq('user_id', user!.id)
        .eq('week_number', week)
        .eq('year', year)
        .maybeSingle()
...
export function useCreateWeekPlan() {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      const { error: dErr } = await supabase.from('weekly_plan_days').insert(days)
      if (dErr) throw dErr
    },
...
export function useCopyPreviousPlan() {
  ...
  return useMutation({
    mutationFn: async (prev: WeeklyPlan) => {
      ...
      const { error } = await supabase
        .from('weekly_plans')
        .insert({ id: planId, user_id: user!.id, week_number: week, year, copied_from_id: prev.id })
      if (error) throw error
      ...
      const days = prev.weekly_plan_days.map((d) => {
        ...
        return { id: newId, weekly_plan_id: planId, day_of_week: d.day_of_week, is_rest_day: d.is_rest_day }
      })
      if (days.length) {
        const { error: dErr } = await supabase.from('weekly_plan_days').insert(days)
        if (dErr) throw dErr
      ...
      if (routineRows.length) {
        const { error: rErr } = await supabase.from('weekly_plan_day_routines').insert(routineRows)
        if (rErr) throw rErr
      }
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['weekly_plan'] }),
...
export function useAddRoutineToDay() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async ({ planDayId, routineId }: { planDayId: string; routineId: string }) => {
...
export function useRemoveRoutineFromDay() {
  ...
  return useMutation({
    mutationFn: async (dayRoutineId: string) => {
      const { error } = await supabase.from('weekly_plan_day_routines').delete().eq('id', dayRoutineId)
      if (error) throw error
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['weekly_plan'] }),
  })
...
export type PlanExercise = {
  ...
  exercises: { id: string; name: string; name_es: string | null; target_muscle: string | null } | null
}

export type PlanRoutineWithExercises = {
  id: string
  routine_id: string
  routines: { id: string; name: string; routine_exercises: PlanExercise[] } | null
}
...
export function useTodayPlanRoutines() {
  ...
  return useQuery({
    queryKey: ['today_plan_routines', user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('weekly_plans')
        .select(`
        ...
        .maybeSingle()
      if (error) throw error
      const planDay = (data?.weekly_plan_days as any)?.[0]
      if (!planDay || planDay.is_rest_day) return null
      return (planDay.weekly_plan_day_routines ?? []) as PlanRoutineWithExercises[]
    },
    enabled: !!user,
  })
}
...
export function useToggleRestDay() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async ({ planDayId, isRest }: { planDayId: string; isRest: boolean }) => {
      if (isRest) {
        await supabase.from('weekly_plan_day_routines').delete().eq('plan_day_id', planDayId)
      }
      const { error } = await supabase
